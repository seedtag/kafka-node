...
KafkaClient.prototype.sendRequest = function (request, callback) {
  ...
  const sendToBroker = async.ensureAsync((payload, leader, callback) => {
    ...
    if (!broker || !broker.isConnected()) {
      ...
      try {
        const buff = Object.values(payloads)[0][0].messages[0].value;
        const snappy = require('snappy');
        snappy.uncompress(buff, { asBuffer: false }, (err, data) => {
          callbacked = true;
          callback(new errors.BrokerNotAvailableError('Broker not available (sendRequest). Payload: ' + data || buff));
        });
      } catch (e) {
        if (!callbacked) callback(new errors.BrokerNotAvailableError('Broker not available (sendRequest). Nopayload'));
      }
      return;
    }
